...
if(connect.readyState == 4 && connect.status == 200){ //Ax
   if(connect.responseText == 1){
    /* conexion exitosa*/
  result1 += '<div class="alert alert-dismissible alert-success">';
  result1 += '<button type="button" class="close" data-dismiss="alert">X</button>';
  result1 += '<h4 class="alert-heading">¡Atención!</h4>';
  result1 += '<p class="mb-0">Conexión exitosa... (login.js)</p>';
  result1 += '</div>';
  __('_AJAX_LOGIN_').innerHTML = result1;
  location.reload();
}else{
  result0 += '<div class="alert alert-dismissible alert-warning">';
  result0 += '<button type="button" class="close" data-dismiss="alert">X</button>';
  result0 += '<h4 class="alert-heading">¡Atención!</h4>';
  result0 += '<p class="mb-0">Usuario y/o Contraseña incorrectos. (login.js)</p>';
  result0 += '</div>';
  __('_AJAX_LOGIN_').innerHTML = result0; //.innerHTML = connect.responseText;  muestra el error de php
}
